...
\begin{tikzpicture}
  ...
  \node[Black, below=20, align=center] at (90:\R) {\LARGE $\mathcal{C}_3$};
  \node[Black, above=20, align=center] at (225:\R) {\LARGE $\mathcal{C}_1$};
  \node[Black, left=20, align=center] at (-5:\R) {\LARGE $\mathcal{C}_2$};

  \draw[<->, very thick, color=Plum] (O2) ++(-0.6,-0.6) -- (-0.6,-0.6) node[midway, below] {\LARGE $\eta_2$};
  \draw[-{Stealth[scale=0.6,inset=0pt]}, very thick, color=Plum] (O) -- ++(145:1.5) node[near end, below left] {\LARGE $- \nabla U^R_{2}$};

  %\draw[densely dotted, thick] (-\R,0) -- (\R,0);% node[right] {$x$};
  %\draw[densely dotted, thick] (0,-\R) -- (0,\R);% node[left] {$y$};

  %\draw[->, thick, color=Plum] (0,0) -- (-170:\R)  node[left] {$\boldsymbol{x}_R$};
